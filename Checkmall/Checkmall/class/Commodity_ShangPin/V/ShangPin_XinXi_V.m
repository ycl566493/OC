...
-(void)init_UI{
    ...
    ///价格
    lbl_JG = [[UILabel alloc]initWithFrame:CGRectMake(15, 0 ,200, 30)];
    lbl_JG.font= [UIFont systemFontOfSize:20];
    lbl_JG.textColor = UIColorFromHex(0xf46c18);
    lbl_JG.text = @"￥9900.00￥2002.00";
    ...
    lbl_TGSL.text = @"已团888份";
    [self addSubview:lbl_TGSL];
    
    //分享按钮
    btn_FX = [[UIButton alloc]initWithFrame:CGRectMake(self.width - 50, 0, 50, 50)];
    ...
    lbL_Title.text = @"火龙果火龙果火龙果火龙果火龙果火龙果火龙果火龙果火";
    ...
    //到达时间
    lbl_RQ = [[UILabel alloc]initWithFrame:CGRectMake(15, btn_Jia.top, btn_Jian.left - 30, btn_Jia.height)];
    lbl_RQ.font = font13;
    lbl_RQ.textColor = UIColorFromHex(0x8c8c8c);
    lbl_RQ.text = @"2019年1月33日22:99到";
...
-(void)setStr_Title:(NSString *)str_Title{
    ...
    //加减数量
    btn_Jia.top = lbL_Title.bottom;
    lbl_SL.top =  btn_Jia.top;
    btn_Jian.top = lbl_SL.top;
    
    //到达时间
    lbl_RQ.top = btn_Jia.top;
    
    //分割线
    view_FGX.top = btn_Jia.bottom + 7;
    
}

+ (CGFloat)get_H:(id)data{
    CGFloat fff = 30 + 20 +10 +24 + 7 + .5;
    NSString *str_T = data;
