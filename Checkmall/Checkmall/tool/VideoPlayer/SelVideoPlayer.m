...
/** 关闭按钮 */
@property (nonatomic , strong)  UIButton *btn_GB;
...
- (instancetype)initWithFrame:(CGRect)frame configuration:(SelPlayerConfiguration *)configuration
{
    ...
    if (self) {
        ...
        self.btn_GB = [[UIButton alloc]initWithFrame:CGRectMake(self.width - 45, 15, 30, 30)];
        [self.btn_GB setImage:[UIImage imageNamed:@"SPGB"] forState:UIControlStateNormal];
        [self.btn_GB addTarget:self action:@selector(btn_GB_Action) forControlEvents:UIControlEventTouchUpInside];
        [self addSubview: self.btn_GB];
    }
...
- (void)_videoZoomInWithDirection:(UIInterfaceOrientation)orientation
{
    ...
    //关闭按钮位置
    self.btn_GB.mj_x = self.height - 45;
...
- (void)_videoZoomOut
{
    ...
    //关闭按钮位置
    self.btn_GB.mj_x = self.width - 45;
